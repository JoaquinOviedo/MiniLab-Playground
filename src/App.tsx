...
const computerKeyMap: Record<string, number> = { a: 48, w: 49, s: 50, e: 51, d: 52, f: 53, t: 54, g: 55, y: 56, h: 57, u: 58, j: 59, k: 60, o: 61, l: 62, p: 63, ';': 64 }
const defaultKnobs = { tone: 58, reverb: 35, delay: 12, attack: 24 }

const copy = {
  es: {
    eyebrow: 'Un lugar para', freePlay: 'MODO LIBRE', heroTitle: 'Haz algo', heroEmphasis: 'pequeño.', heroCopy: 'Conecta tu MiniLab, elige un sonido y empieza a tocar.', tempo: 'TEMPO', bpm: 'BPM', slower: 'Más lento', faster: 'Más rápido', chooseSound: 'ELIGE UN SONIDO', soundSettings: 'ajustes', inspire: 'Inspírame',
  ...
  en: {
    eyebrow: 'A tiny place to', freePlay: 'FREE PLAY', heroTitle: 'Make a little', heroEmphasis: 'something.', heroCopy: 'Connect your MiniLab, choose a sound, and start playing.', tempo: 'TEMPO', bpm: 'BPM', slower: 'Slower', faster: 'Faster', chooseSound: 'CHOOSE A SOUND', soundSettings: 'settings', inspire: 'Inspire me',
...
function App() {
  ...
  useEffect(() => {
    ...
    const onKeyUp = (event: KeyboardEvent) => { const note = computerKeyMap[event.key.toLowerCase()]; if (note !== undefined) releaseNote(note) }
  ...
  const pressNote = (note: number, velocity = 96) => { void audio.start().then(() => { audio.noteOn(note, velocity); setIsAudioReady(true) }); setActiveNotes((previous) => new Set(previous).add(note)); setTip(`${midiNoteName(note)} · ${language === 'es' ? 'teclado del ordenador' : 'computer keyboard'}`) }
